    ...
    T: GeoFloat + FloatConst + Signed,
{
    ...
    let ymin1 = GenPoint(poly1.exterior().0[poly1_extremes.y_min.index]);
    let ymax2 = GenPoint(poly2.exterior().0[poly2_extremes.y_max.index]);
    ...
    T: GeoFloat + Signed,
{
    ...
    let clockwise = GenPoint(pprev).cross_prod(GenPoint(p.0), GenPoint(pnext)) < T::zero();
    let slope_prev;
    let slope_next;
    ...
    T: GeoFloat + FloatConst + Signed,
{
    ...
    let triarea = Triangle::from([p, punit, GenPoint(pnext)]).signed_area();
    let edgelen = p.euclidean_distance(&GenPoint(pnext));
    ...
    let left = leftturn(p, perpunit, GenPoint(pnext));
    if left == 0 {
        obtuse = true;
    }
    if clockwise {
    ...
    T: GeoFloat + FloatConst + Signed,
{
    ...
    if (state.ap1 - minangle).abs() < T::from(0.002).unwrap() {
        ...
        state.p1next = GenPoint(state.poly1.exterior().0[p1next]);
        state.p1_idx = p1next;
        state.alignment = Some(AlignedEdge::VertexP);
    }
    if (state.aq2 - minangle).abs() < T::from(0.002).unwrap() {
        ...
        state.q2next = GenPoint(state.poly2.exterior().0[q2next]);
        state.q2_idx = q2next;
