    ...
    T: CoordFloat + RTreeNum,
{
    ...
    while let Some(mut smallest) = pq.pop() {
        ...
        let left_point = GenPoint(orig.0[left as usize]);
        let middle_point = GenPoint(orig.0[smallest.current]);
        let right_point = GenPoint(orig.0[right as usize]);
    ...
    tree.locate_in_envelope_intersecting(&rstar::AABB::from_corners(br, tl))
        .any(|c| {
            ...
            ca.0 != point_a
                ...
                && cb.0 != point_a
                && cb.0 != point_c
                && cartesian_intersect(ca, cb, GenPoint(point_a), GenPoint(point_c))
        })
}
